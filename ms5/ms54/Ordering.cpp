...
namespace seneca {
    ...
    void Ordering::printTotals(std::ostream& ostr, double total) const {
        // Convert to cents to avoid floating point rounding issues
        int totalCents = static_cast<int>(total * 100 + 0.5);  // Added 0.5 for proper rounding
        int taxCents = static_cast<int>(total * Tax * 100 + 0.5);  // Added 0.5 for proper rounding
        int totalWithTaxCents = totalCents + taxCents;
