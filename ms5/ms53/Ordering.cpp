...
namespace seneca {
    ...
    void Ordering::printTotals(std::ostream& ostr, double total) const {
        ...
        int taxInt = int(total * Tax * 100);
        int totalTaxInt = int(total * (1 + Tax) * 100);

        ostr << "                     Total:        "
            << totalInt / 100 << "." << (totalInt % 100 < 10 ? "0" : "") << totalInt % 100 << endl;
        ostr << "                     Tax:           "
            << taxInt / 100 << "." << (taxInt % 100 < 10 ? "0" : "") << taxInt % 100 << endl;
        ostr << "                     Total+Tax:    "
            << totalTaxInt / 100 << "." << (totalTaxInt % 100 < 10 ? "0" : "") << totalTaxInt % 100 << endl;
        ostr << "========================================" << endl;
    }
